...
def _row(*, summary: dict, verdict: str = "YES") -> dict:
    return {
        ...
        "confidence": 0.9,
        "evidence": [{"ref": "round:1:0"}],
        "price_series": [{"round": 1, "price_yes": 0.6}],
...
def test_us226_owner_missing_fails_closed_after_public_access_check(app: Flask, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(api.sbc, "get_simulation_owner", lambda _sim: None)
...
def test_us226_owner_read_error_fails_closed(app: Flask, monkeypatch: pytest.MonkeyPatch) -> None:
    _allow(monkeypatch, [])
    monkeypatch.setattr(api.sbc, "get_simulation_owner", lambda _sim: (_ for _ in ()).throw(RuntimeError("offline")))
...
def test_us226_resolution_query_is_scoped_to_canonical_owner_and_exposes_consistent_wealth(app: Flask, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(api.sbc, "get_simulation_owner", lambda _sim: {"org_id": "org-1"})
    monkeypatch.setattr(api.sbc, "get_supabase_admin", lambda: client)
...
def test_us226_pending_or_divergent_summaries_never_expose_arbitrary_wealth(app: Flask, monkeypatch: pytest.MonkeyPatch, summaries: list[dict]) -> None:
    ...
    monkeypatch.setattr(api.sbc, "get_simulation_owner", lambda _sim: {"org_id": "org-1"})
    monkeypatch.setattr(api.sbc, "get_supabase_admin", lambda: _Client(rows))
...
def test_us226_supabase_failure_fails_closed(app: Flask, monkeypatch: pytest.MonkeyPatch) -> None:
    _allow(monkeypatch, [])
    monkeypatch.setattr(api.sbc, "get_simulation_owner", lambda _sim: {"org_id": "org-1"})
    monkeypatch.setattr(api.sbc, "get_supabase_admin", lambda: (_ for _ in ()).throw(RuntimeError("offline")))
...
def test_us226_api_malformed_verdict_never_exposes_final_wealth(
    ...
    row["verdict"] = verdict
    monkeypatch.setattr(api.sbc, "get_simulation_owner", lambda _sim: {"org_id": "org-1"})
    monkeypatch.setattr(api.sbc, "get_supabase_admin", lambda: _Client([row]))

    data = app.test_client().get("/api/report/sim-1/market-resolutions").get_json()["data"]

    assert data["complete"] is False
    assert data["final_wealth"] == []
