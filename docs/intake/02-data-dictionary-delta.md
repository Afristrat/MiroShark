...
| `transcript` | jsonb | NULL | Échanges de l'étape B : `[{role, content, ts}]` — donnée personnelle, cf. rétention |
...
## Rétention (cf. 07-legal-compliance)

- `transcript` : purge automatique à J+90 après `completed_at` (pg_cron), le `brief`
  (données minimisées) est conservé avec le devis.
...
- Sessions `abandoned` : purge complète à J+30.
