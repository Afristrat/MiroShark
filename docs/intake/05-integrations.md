...
| **Cal.com self-hosted** (`agenda.ai-mpower.com`, DÉJÀ en production — ADR-IQ-03 v3) | Branche entretien : lien de réservation vers l'event type « Entretien Bassira » (slug `entretien-bassira-20-min`, id 25, 20 min), localisé selon la langue de session | ⚠️ **API appelée via le hostname PUBLIC dédié** `https://api-agenda.ai-mpower.com/v2/...` (PAS `agenda.ai-mpower.com/api/v2` → Cloudflare « DNS points to prohibited IP »). Clé : env Coolify `CALCOM_API_KEY` (POST envs = JSON minimal `{key,value}`). Endpoints utiles : `GET /v2/me` (validation), `GET /v2/event-types`, `GET /v2/slots`. Le lien de réservation envoyé au prospect reste public (page web Cal.com) : `https://agenda.ai-mpower.com/a.mansouri/entretien-bassira-20-min`. |
